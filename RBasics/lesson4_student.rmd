...
ggplot(aes(x = age, y = friend_count), data = pf) + geom_jitter(alpha = 1/20, position = position_jitter(h = 0)) +
  xlim(13,90) +
  coord_trans(y = 'sqrt')
```
...
pf.fc_by_age <- summarise(age_by_group,
...
#subset
with(subset(pf,pf$age<=70 & pf$age>=13),cor.test(friend_count,age, method = "pearson"))
```
...
with(subset(pf,pf$age<=70 & pf$age>=13), cor.test(age, friend_count))
```
...
with(subset(pf,pf$www_likes_received <= quantile(pf$www_likes_received,0.95)), cor(www_likes_received, likes_received))
```
